...
// Core dynamic discovery loop in kernel.js
const fs = require('fs');
...
function mountFeatures(app) {
  ...
  dirs.forEach(dir => {
    if (dir.startsWith('_')) return; // Skip templates

    const manifestPath = path.join(featuresDir, dir, 'feature.manifest.json');
    if (fs.existsSync(manifestPath)) {
      try {
        const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
        if (manifest.enabled) {
          const routesPath = path.join(featuresDir, dir, manifest.routes.file);
          const router = require(routesPath);
          app.use(manifest.routes.prefix, router);
          console.log(`[Kernel Mounted]: ${manifest.name} -> ${manifest.routes.prefix}`);
        }
      } catch (err) {
        console.error(`[Kernel] Failed to mount feature in ${dir}:`, err.message);
      }
    }
  });
}
